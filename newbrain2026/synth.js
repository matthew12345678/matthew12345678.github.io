(() => {
  ...
  class Voice {
    ...
    process(bufferL, bufferR, n, sampleRate) {
      ...
      const g = this.gain;
      const rate = Math.max(0.01, p.totalSpeed);
      ...
      for (let i = 0; i < n; i++) {
        ...
        const freqMs =
          (p.freqSpeed * mapBipolar(tabosc4(t3, ph3), p.freqFreqLower, p.freqFreqUpper)) / rate;
        const volMs =
          (p.volSpeed * mapBipolar(tabosc4(t4, ph4), p.volFreqLower, p.volFreqUpper)) / rate;
