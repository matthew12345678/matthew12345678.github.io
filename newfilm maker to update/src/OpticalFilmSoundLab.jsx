...
function scanMetrics(scan, p) {
  const len = scan.length || 1;
  let sum = 0;
  let sumSq = 0;
  let peak = 0;
  let num = 0;
  ...
  for (let i = 0; i < len; i++) {
    ...
    num += i * v;
    den += v;
    if (i > 0) rough += Math.abs(v - scan[i - 1] / 255);
  }

  const avg = sum / len;
  const rms = Math.sqrt(sumSq / len);
  const centroid = den ? num / den / len : 0.5;
  const fundamental = 40 + centroid * p.freq * 6 * p.sensitivity;
  const playbackRate = 2 ** ((centroid - 0.5) * 2.2 * p.sensitivity);
  const cutoff = 140 + avg * 10000 * p.sensitivity;
  const highpass = 30 + (1 - avg) * 500;
  const resonance = Math.min(22, 0.4 + peak * 16 * p.sensitivity);
  const pan = Math.max(-1, Math.min(1, (centroid - 0.5) * 1.8));
  const drive = Math.min(1, (rough / len) * 10 * p.sensitivity);

  const harmonicAmps = [];
  for (let h = 0; h < MAX_HARMONICS; h++) {
    const idx = Math.min(len - 1, Math.floor((h / MAX_HARMONICS) * len));
    harmonicAmps.push(scan[idx] / 255);
  }
...
export default function OpticalFilmSoundLab() {
  ...
  const [soundModEnabled, setSoundModEnabled] = useState(false);
  const [modPitch, setModPitch] = useState(true);
  const [modTape, setModTape] = useState(false);
  const [modVolume, setModVolume] = useState(true);
  const [modFilter, setModFilter] = useState(true);
  const [modHarmonics, setModHarmonics] = useState(true);
  const [modPan, setModPan] = useState(true);
  const [modCrush, setModCrush] = useState(false);
  const [sampleMix, setSampleMix] = useState(0.85);
  const [harmonicMix, setHarmonicMix] = useState(0.45);
  const [synthMix, setSynthMix] = useState(0.3);
  const [syncSampleToFilm, setSyncSampleToFilm] = useState(false);
  ...
  const ensureAudio = useCallback(async () => {
    ...
    const samplePanner = ctx.createStereoPanner();
    const sampleGain = ctx.createGain();
    sampleGain.gain.value = 0;
    const sampleHighpass = ctx.createBiquadFilter();
    sampleHighpass.type = "highpass";
    sampleHighpass.frequency.value = 80;
    const sampleFilter = ctx.createBiquadFilter();
    sampleFilter.type = "lowpass";
    sampleFilter.frequency.value = 2400;
    sampleFilter.Q.value = 1;
    const sampleShaper = ctx.createWaveShaper();
    sampleShaper.curve = makeWaveshaperCurve(0);
    sampleShaper.oversample = "2x";

    samplePanner.connect(sampleGain);
    sampleGain.connect(sampleHighpass);
    sampleHighpass.connect(sampleFilter);
    sampleFilter.connect(sampleShaper);
    sampleShaper.connect(master);
  ...
  const connectSampleElement = useCallback(async () => {
    await ensureAudio();
    const ctx = audioRef.current;
    const n = nodesRef.current;
    const el = sampleAudioRef.current;
    if (!ctx || !n || !el || n.sampleMediaSource) return;

    n.sampleMediaSource = ctx.createMediaElementSource(el);
    n.sampleMediaSource.connect(n.samplePanner);
  }, [ensureAudio]);
  ...
  const setModeGains = useCallback((active) => {
    ...
    const synthLevel = active ? p.synthMix : 0;

    n.mainGain.gain.value =
      active && (scanModes.includes(m) || brightModes.includes(m)) ? 0.35 * synthLevel : 0;
    n.rgbGain.gain.value = active && rgbModes.includes(m) ? 0.25 * synthLevel : 0;
    n.noiseGain.gain.value = active && noiseModes.includes(m) ? 0.2 * synthLevel : 0;
  }, []);

  const syncSamplePlayback = useCallback((withFilm) => {
    const el = sampleAudioRef.current;
    const video = videoRef.current;
    if (!el || !hasSampleRef.current) return;

    if (withFilm && video?.duration && el.duration) {
      el.currentTime = (video.currentTime / video.duration) * el.duration;
    }
    el.playbackRate = withFilm && video ? video.playbackRate : 1;
  }, []);

  const playSample = useCallback(async () => {
    const el = sampleAudioRef.current;
    if (!hasSampleRef.current || !el) return;
    el.loop = true;
    syncSamplePlayback(paramsRef.current.syncSampleToFilm);
    try {
      await el.play();
    } catch {
      /* needs user gesture */
  ...
  const updateSampleModulation = useCallback((scan) => {
    const ctx = audioRef.current;
    const n = nodesRef.current;
    const p = paramsRef.current;
    if (!ctx || !n || !p.soundModEnabled || !hasSampleRef.current) return;

    const m = scanMetrics(scan, p);
    const t = ctx.currentTime;

    if (p.modVolume) {
      const vol = Math.max(0.02, Math.min(1, (0.15 + m.rms * 1.4) * p.sampleMix));
      n.sampleGain.gain.setTargetAtTime(vol, t, MOD_SLEW);
    } else {
      n.sampleGain.gain.setTargetAtTime(p.sampleMix, t, MOD_SLEW);
    }

    if (p.modFilter) {
      n.sampleFilter.frequency.setTargetAtTime(
        Math.max(80, Math.min(18000, m.cutoff)),
        t,
        MOD_SLEW,
      );
      n.sampleFilter.Q.setTargetAtTime(m.resonance, t, MOD_SLEW);
      n.sampleHighpass.frequency.setTargetAtTime(
        Math.max(20, Math.min(2000, m.highpass)),
        t,
        MOD_SLEW,
      );
    }
    ...
    if (p.modPan) {
      n.samplePanner.pan.setTargetAtTime(m.pan, t, MOD_SLEW);
    }

    const el = sampleAudioRef.current;
    if (el && (p.modPitch || p.modTape)) {
      let rate = 1;
      if (p.modTape) rate = m.playbackRate;
      if (p.modPitch) {
        rate *= 2 ** ((m.centroid - 0.5) * 1.2 * p.sensitivity);
      }
      if (!p.syncSampleToFilm) {
        el.playbackRate = Math.max(0.25, Math.min(4, rate));
      }
    }

    if (p.modCrush) {
      n.sampleShaper.curve = makeWaveshaperCurve(m.drive);
    }

    if (p.modHarmonics) {
      const fund = p.quantize ? quantizeFreq(m.fundamental) : m.fundamental;
      for (let i = 0; i < n.harmonicOscs.length; i++) {
        const h = i + 1;
        n.harmonicOscs[i].frequency.setTargetAtTime(fund * h, t, MOD_SLEW);
        const amp = (m.harmonicAmps[i] ?? 0) * p.harmonicMix * 0.22 * m.avg;
        n.harmonicGains[i].gain.setTargetAtTime(amp, t, MOD_SLEW);
      }
    } else {
      for (const g of n.harmonicGains) {
        g.gain.setTargetAtTime(0, t, MOD_SLEW);
      }
    }
  }, []);
  ...
  }, [setModeGains, updateAudio, updateSampleModulation]);
  ...
  const seekFilm = useCallback((pos) => {
    ...
    if (paramsRef.current.syncSampleToFilm) {
      const el = sampleAudioRef.current;
  ...
  const onLoadSound = useCallback(
    async (e) => {
      ...
      patchParams({ soundModEnabled: true });
      setStatus(`Sound loaded: ${file.name} — enable modulation and press Play.`);
      e.target.value = "";
    },
    [ensureAudio, connectSampleElement],
  );
  ...
  const play = useCallback(async () => {
    ...
    if (still) {
      ...
      setPlaying(true);
      if (hasSampleRef.current && paramsRef.current.soundModEnabled) {
        await playSample();
      }
  ...
  }, [ensureAudio, setModeGains, playSample]);
  ...
  const pause = useCallback(() => {
    ...
    setModeGains(false);
    const n = nodesRef.current;
    if (n) n.sampleGain.gain.value = 0;
    setStatus("Paused.");
  }, [setModeGains, pauseSample]);
  ...
  const stopAll = useCallback(() => {
    ...
    const n = nodesRef.current;
    if (n) {
      n.sampleGain.gain.value = 0;
      for (const g of n.harmonicGains) g.gain.value = 0;
    }
    setStatus("Stopped.");
  }, [setModeGains, stopRecording, pauseSample]);
  ...
  }, [ensureAudio, setModeGains, stopRecordingInternal, playSample]);

  useEffect(() => {
    const video = videoRef.current;
    if (video?.src) video.playbackRate = speed;
    if (paramsRef.current.syncSampleToFilm) {
      syncSamplePlayback(true);
    }
  }, [speed, syncSamplePlayback]);

  useEffect(() => {
    const n = nodesRef.current;
    if (!n) return;
    if (!soundModEnabled || !hasSample) {
      n.sampleGain.gain.value = 0;
      for (const g of n.harmonicGains) g.gain.value = 0;
    }
  }, [soundModEnabled, hasSample]);
  ...
  return (
    <div className="w-full min-h-screen bg-black text-white p-6 font-sans">
        ...
        <div className="bg-zinc-900 rounded-3xl p-4 shadow-2xl border border-zinc-800 overflow-y-auto max-h-[85vh]">
          ...
          <div className="space-y-4">
            ...
            <div className="border-t border-zinc-700 pt-4 space-y-4">
              <h3 className="text-lg font-semibold">Sound Modulation</h3>
              <p className="text-xs text-zinc-500">
                Load audio and let the scan wave reshape it in real time.
                {sampleName ? ` Loaded: ${sampleName}` : ""}
              </p>

              <label className="bg-zinc-100 text-black px-3 py-2 rounded-xl cursor-pointer hover:bg-white transition inline-block text-sm">
                Load Sound
                <input
                  type="file"
                  accept="audio/*"
              ...
              <label className="flex items-center gap-2 text-sm">
                <input
                  type="checkbox"
                  checked={soundModEnabled}
                  disabled={!hasSample}
                  onChange={(e) => {
                    const value = e.target.checked;
                    patchParams({ soundModEnabled: value });
                    setSoundModEnabled(value);
                  }}
                />
                Enable optical modulation
              </label>

              <div className="grid grid-cols-2 gap-2 text-sm">
                {[
                  ["modVolume", "Volume", modVolume, setModVolume],
                  ["modFilter", "Filter", modFilter, setModFilter],
                  ["modPitch", "Pitch drift", modPitch, setModPitch],
                  ["modTape", "Tape speed", modTape, setModTape],
                  ["modHarmonics", "Harmonics", modHarmonics, setModHarmonics],
                  ["modPan", "Stereo pan", modPan, setModPan],
                  ["modCrush", "Wave crush", modCrush, setModCrush],
                ].map(([key, label, checked, setter]) => (
                  <label key={key} className="flex items-center gap-2">
                    <input
                      type="checkbox"
                      checked={checked}
                      disabled={!soundModEnabled}
                      onChange={(e) => {
                        const value = e.target.checked;
              ...
              <label className="flex items-center gap-2 text-sm">
                <input
                  type="checkbox"
                  checked={syncSampleToFilm}
                  disabled={!hasSample}
                  onChange={(e) => {
                    const value = e.target.checked;
                    patchParams({ syncSampleToFilm: value });
                    setSyncSampleToFilm(value);
                  }}
                />
                Sync sound position to film
              </label>

              <div>
                <label className="block mb-1 text-sm text-zinc-400">
                  Sample level ({sampleMix.toFixed(2)})
                </label>
                <input
                  type="range"
                  min={0}
              ...
              <div>
                <label className="block mb-1 text-sm text-zinc-400">
                  Harmonic layer ({harmonicMix.toFixed(2)})
                </label>
                <input
                  type="range"
                  min={0}
              ...
              <div>
                <label className="block mb-1 text-sm text-zinc-400">
                  Synth oscillator mix ({synthMix.toFixed(2)})
                </label>
                <input
                  type="range"
                  min={0}
      ...
      <canvas
        ...
        aria-hidden
      />
      <audio ref={sampleAudioRef} className="hidden" preload="auto" loop />
    </div>
  );
}
